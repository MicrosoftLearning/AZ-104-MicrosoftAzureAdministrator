...
1. From the Cloud Shell pane, run the following to deploy the virtual machine by using the uploaded template and parameter files:
   ...
   ```
    ...
    > 1. Click on the `{}` button in your CloudShell, select the **az104-07-vms-loop-parameters.json** from the left hand side bar and take a note of the `vmSize` parameter value.
